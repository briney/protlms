...
#
# Background: torch._inductor (imported transitively by flex_attention, which
# E1 uses instead of flash-attn) calls getpass.getuser() to build its cache
# directory path. When running with --user uid:gid and the uid is not in the
# container's /etc/passwd, getpwuid() raises KeyError. libnss_wrapper shims
# the NSS layer to return a synthetic entry for the current uid.
